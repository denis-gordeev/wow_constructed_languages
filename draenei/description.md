# Draenei Language — Overview

## Speakers

The Draenei language is spoken by the Draenei, a faction of the Eredar species who refused to join the Burning Legion. Draenei means "Exiled Ones" (*draen* "exile/refuge" + *ei* "ones/people"). The language is used by Draenei NPCs throughout World of Warcraft, particularly in The Burning Crusade, Warlords of Draenor, and Legion expansions.

## History

The Eredar were the original inhabitants of the planet Argus. When Sargeras tempted the Eredar leaders (Kil'jaeden and Archimonde) with power, the prophet Velen led a dissenting faction away from corruption. These exiles adopted the name *Draenei* ("Exiled Ones") and fled Argus aboard the dimensional ship *Oshu'gun*, guided by the Naaru *K'ure*.

After a long journey across worlds, the Draenei settled on a planet they named *Draenor* ("Exiles' Refuge"). They built cities and temples there, including *Shattrath* ("Dwelling of Light"), *Karabor* (the Black Temple), *Auchindoun*, and *Telredor*. When the Orcs were corrupted by the Burning Legion and waged war on the Draenei, most were killed; survivors included the unmutated Draenei and the *Krokul* ("Broken"), Draenei devolved by fel energy.

The Draenei later crashed on Azeroth aboard *The Exodar* (a wing of the naaru vessel *Tempest Keep*) and joined the Alliance. During the Legion expansion, the Draenei returned to Argus aboard the *Vindicaar*.

## Relationship to Eredar

Draenei is essentially the same language as Eredar. The split between Draenei and Man'ari Eredar is cultural and religious, not linguistic. The Draenei use the term *Man'ari* ("wrong/unnatural/corrupt/damned") to refer to the Eredar who accepted demonic corruption, and this term itself is a Draenei/Eredar word. Place names on Argus and Draenor share morphological patterns (e.g., the *-or* toponymic suffix in *Draenor*, possibly *Telredor*).

The key linguistic divergence is lexical: the Draenei have developed religious vocabulary centered on the Light and the Naaru (*Sha'tar*, *Shattrath*, *Aldor*), while the Man'ari Eredar may retain older or corrupted terms. The extent of this divergence is unknown due to limited attestation of both varieties.

## Cultural Context

Draenei culture is centered on worship of the Light and reverence for the Naaru. This religious orientation is embedded in the language:

- *Shattrath* = "Dwelling of Light" — the name of their greatest city.
- *Sha'tar* = "Born of Light" — the Naaru faction that reclaimed Shattrath.
- *Aldor* = possibly "holy ones" or "devoted" — a Draenei religious order.
- *Kara* = "temple" / "holy place" — appears in *Karabor*.

The Draenei language reflects a worldview in which Light, exile, and refuge are central themes. The self-designation *Draenei* ("Exiled Ones") and the planet name *Draenor* ("Exiles' Refuge") encode the trauma of their flight from Argus as foundational identity.

## Key Phonological Features

- **Guttural consonants**: /k/, /g/, and possibly /x/ (represented as *kh*) are frequent, giving the language a resonant, sometimes harsh quality.
- **Consonant clusters**: Complex onsets and codas are permitted (e.g., *Shattrath* /ʃtr/, *Krokul* /kr/).
- **Apostrophe**: Marks morpheme boundaries and possibly a glottal stop /ʔ/.
- **Vowel richness**: All five cardinal vowels /a e i o u/ are attested, along with possible diphthongs.

## Attestation and Reliability

Draenei is a **poorly attested** constructed language. The known corpus consists of:

- A small set of glossed proper nouns (place names, group names).
- Three unglossed phrases (*Kehla men'or*, *Pheta thones gamera*, *Dioniss aca*).
- A handful of individual words with uncertain meaning (*Khad*, *Ata*).

No grammar, syntax, or extended text has been published by Blizzard. All grammatical inferences are based on morphological decomposition of compound words. No sentences with reliable translations exist.

### What is reliable

- The roots *draen* ("exile/refuge"), *ei* ("ones/people"), and *or* ("world/land") are well-supported by Blizzard's official translations of *Draenei* and *Draenor*.
- *Man'ari* = "wrong/corrupt" is confirmed by in-game usage.
- *Sha'tar* = "Born of Light" and *Shattrath* = "Dwelling of Light" are confirmed by Blizzard.

### What is uncertain

- The decomposition of *Shattrath* into *shat* + *trath* and the assignment of "dwelling" and "light" to these roots respectively (it is possible that *sha* = "light" and *ttrath* = "dwelling," or that the segmentation is different).
- The meaning of *kara* as "temple" (inferred from *Karabor*).
- The meaning of all unglossed phrases.
- The meaning of Naaru names (*A'dal*, *K'ure*).
- Any phonological claims, which are based solely on English-language orthography.

### What is unknown

- Syntax (word order, case marking, agreement).
- Verbal morphology.
- Pronouns, prepositions, or functional elements.
- Any vocabulary beyond the words listed in `vocabulary.md`.

## References

- World of Warcraft: The Burning Crusade (2007). Blizzard Entertainment.
- World of Warcraft: Warlords of Draenor (2014). Blizzard Entertainment.
- World of Warcraft: Legion (2016). Blizzard Entertainment.
- Blizzard Entertainment. Various official publications and in-game text.
