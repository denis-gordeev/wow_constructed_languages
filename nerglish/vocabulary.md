# Nerglish (Murloc/Jinyu/Ankoan/Kobyss) Vocabulary

## Language Names

| Name | Speakers | Notes |
|------|----------|-------|
| Nerglish | Murlocs, makrura | The murloc language; shared with makrura |
| (no name attested) | Jinyu | Evolved from Nerglish but no separate name attested |
| (no name attested) | Ankoan | Evolved from jinyu but no separate name attested |
| (no name attested) | Kobyss | Near-total language loss; no native vocabulary |

## Murloc (Nerglish)

### Self-Designations & Vocalizations

| Word | Translation | Notes |
|------|-------------|-------|
| gmmmlmrmrgmg | "murloc" (self-designation) | Attested murloc-language word for "murloc" |
| Mmmurlok | "murloc" (self-designation) | Alternative self-designation |
| Aaaaaughibbrgubugbugrguburgle! | (battle cry) | Blizzard's official spelling of murloc battle cry |

### Murloc Songs (Translated Titles)

| Title | Notes |
|-------|-------|
| Fish in the Stream | Attested murloc song (translated to Common) |
| My Favorite Pearl | Attested murloc song (translated to Common) |

### Murloc Proper Names

| Name | Pattern | Notes |
|------|---------|-------|
| Brglmurgl | *Brgl-* + *-murgl* | Winterfin murloc |
| Cleaver Bmurglbrm | *Bmurglbrm* | Winterfin cook |
| Glrglrglr | *Glrgl-* + *-rglr* | Winterfin captive |
| Gmurgl | *Gm-* + *-urgl* | Saltscale murloc |
| Keymaster Urmgrgl | *Urmgrgl* | Winterfin jailor |
| Lurgglbr | *Lurggl-* + *-br* | Winterfin captive |
| Mmmrrrggglll | *Mmmrrrggg-* + *-lll* | Grimscale chieftain |
| Mrmrglmr | *Mr-* + *-mrgl-* + *-mr* | Winterfin murloc |
| Murgurgula | *Mur-* + *-gurg-* + *-ula* | Siltfin murloc |

## Jinyu

### Confirmed Translations

| Word | Translation | Notes |
|------|-------------|-------|
| Onoho | Deep Shadow | Only known jinyu word with confirmed Common translation; from Razorfin tribe elder |

### Jinyu Proper Names

| Name | Pattern | Notes |
|------|---------|-------|
| Aethis | Simple CV(C)CV | Pandarian champion |
| Ashyo | CVCCV | Tian Monastery monk (Clever Ashyo) |
| Cournith | CVCVC | Pandarian champion (Cournith Waterstrider) |
| Eshelon | CVCVCV | Pandarian champion |
| Fujin | CVCV | Pearlfin pearlkeeper (Pearlkeeper Fujin) |
| Gorai | CV.CV | Inkgill waterspeaker (Waterspeaker Gorai) |
| Husshun | CVC.CVC | Tian Monastery trainee |
| Izissha | CV.CVC.CV | Crane Wing monk (Dextrous Izissha) |
| Jogu | CVCV | Tiller member (Jogu the Drunk) |
| Karasshi | CV.CVC.CV | Pearlfin warrior (Bold Karasshi) |
| Kuru | CVCV | Shrine caretaker (Kuru the Light-Hearted) |
| Lu | CV | SI:7 guide (Little Lu) |
| Lusshan | CVC.CVC | Pearlfin leader (Elder Lusshan) |
| Mari | CVCV | Temple of Jade Serpent boss (Wise Mari) |
| Nalash | CV.CVC | Pandarian champion (Nalash Verdantis) |
| Nali | CV.CV | BfA trader (Traveler Nali) |
| Orachi | CV.CV.CV | Inkgill guard captain |
| Ot-Temmdo | CVC.CVCCV | Pearlfin watersmith |
| Rassan | CVC.CVC | Anglers elder (Elder Fisherman Rassan) |
| Rassharom | CVC.CV.CVC | Ancient jinyu Emperor of Pandaria |
| Resshi | CVC.CV | Lorewalkers representative (Scrollmaker Resshi) |
| Sahn | CVC | Pandarian champion (Sahn Tidehunter) |
| Sele'na | CV.CV'.CV | Contains apostrophe (unique among jinyu) |
| Shai Hu | CV + CV | Inkgill corrupter |
| Shushen | CVC.CVC | Fallsong waterspeaker (Tired Shushen) |
| Zai | CV | Inkgill pandarian champion (Zai the Outcast) |
| Zusshi | CVC.CV | Pet tamer (Seeker Zusshi) |
| Ryuli | CCV.CV | Legion-era waterspeaker (Waterspeaker Ryuli) |

### Jinyu Place Names

| Place | Notes |
|-------|-------|
| Pearlfin Village | English compound (tribe name + "village") |
| Inkgill Mere | English compound |
| Fallsong Village | English compound |
| Mezzamere | Possibly non-English; *Mezza-* + *-mere* (cf. English "mere" = lake) |

### Jinyu Titles

| Title | Notes |
|-------|-------|
| Waterspeaker | English compound; jinyu spiritual leader |
| Pearlkeeper | English compound; pearl guardian |
| Deep Oracle | English compound (Razorfin tribe) |

## Ankoan

### Ankoan Proper Names

| Name | Pattern | Notes |
|------|---------|-------|
| Okani | CV.CV.CV | Waveblade leader (Blademaster Okani) |
| Inowari | CV.CV.CV.CV | 4-syllable name (Bladesman Inowari) |
| Ori | CV.CV | Short name (Farseer Ori) |
| Akana | CV.CV.CV | 3-syllable name (Hunter Akana) |
| Kotori | CV.CV.CV | Hearthstone paladin (Kotori Lightblade) |

### Ankoan Place and Tribe Names

| Name | Notes |
|------|-------|
| Mezzamere | Ankoan/Alliance base in Nazjatar; shared with jinyu |
| Waveblade | English compound; the only ankoan clan that took the fight to Azshara |

## Kobyss

### Attested Kobyss Speech (Common/English)

| Phrase | Notes |
|--------|-------|
| "Animate bones" | Kobyss speech; Common, not native language |
| "Devour" | Kobyss speech; Common, not native language |
| "Violence" | Kobyss speech; Common, not native language |

### Kobyss Chieftain Names (TWW: Hallowfall)

| Name | Tribe | Notes |
|------|-------|-------|
| Dreadfang the Merciless | Darkwater | Morgaen's Tears |
| Grand Gluttony | Gutter | Salien Gutters, Azj-Kahet |
| Sharpscale the Devourer | Hungering | Hungering Pool |
| Chieftain Gillcarver | Sunless | Sunless Strand |
| Lightwalker | Deepfin (murloc) | Surface kobyss in Zul'Aman leading deep sea murlocs |

All kobyss chieftain names are Common/English compounds or descriptors — no attested kobyss-language proper names.

### Kobyss Tribal Designations (Complete List)

| Tribe | Leader | Location | Notes |
|-------|--------|----------|-------|
| Darkwater | Dreadfang the Merciless | Morgaen's Tears | |
| Fathomless | Unknown | The Fangs | |
| Gutter | Grand Gluttony | Salien Gutters, Azj-Kahet | Also found in nerubian territory |
| Hungering | Sharpscale the Devourer | Hungering Pool | |
| Murkfin | Unknown | Velhan's Claim | |
| Ragefin | Unknown | Veneration Grounds | Allied with Order of Night |
| Sunless | Chieftain Gillcarver | Sunless Strand | |

### Kobyss NPC Types (Common/English Descriptors)

| Name | Notes |
|------|-------|
| Bristlespine | Descriptive compound; Velhan's Claim |
| Droopfin | Descriptive compound; Velhan's Claim |
| Sharpfang | Descriptive compound; Velhan's Claim |
| Grimslice | Descriptive compound; Veneration Grounds |
| Kobyss Fighter | Generic NPC type |
| Kobyss Trickster | Generic NPC type |
| Kobyss Carver | Generic NPC type |
| Kobyss Shadeshaper | Disguised as Arathi Captive |
| Kobyss Swordsnapper | Bleak Sand; fights both Void and Order of Night |
| Murkfin Depthstalker | Murkfin tribe variant |
| Murkfin Ravager | Murkfin tribe variant |
| Murkfin Sciomancer | Murkfin tribe spellcaster |
| Ragefin Carver | Ragefin tribe variant |
| Ragefin Executioner | Ragefin tribe variant |
| Ragefin Necromancer | Ragefin tribe spellcaster |
| Ragefin Shadowcaster | Ragefin tribe spellcaster |
| Hungering Necromage | Hungering tribe spellcaster |
| Hungering Shimmerfin | Hungering tribe variant |
| Hungering Slicer | Hungering tribe variant |
| Fathomless Ambusher | Fathomless tribe variant |
| Fathomless Necromancer | Fathomless tribe spellcaster |
| Fathomless Warrior | Fathomless tribe variant |
| Fathomless Scout | Fathomless tribe variant |
| Darkwater Clawsmasher | Darkwater tribe; seen with makrura |
| Darkwater Tidecrusher | Darkwater tribe; Veneration Grounds |
| Darkwater Ranger | Darkwater tribe ranger |
| Bleak Beak-Carver | Bleak tribe variant |
| Bleak Beastbreaker | Bleak tribe variant |
| Bleak Claw-Eater | Bleak tribe variant |
| Bleak Flesh-Taker | Bleak tribe variant |
| Bleak Seatamer | Bleak tribe variant |

### Named Kobyss (Proper Names?)

| Name | Notes |
|------|-------|
| Fallowbarb | Possibly a proper name; Shadowmire cavern |
| Sloshmuck | Possibly a proper name; Shadowmire cavern |

### Hallowfall Places in Kobyss Territory

All place names are Common/English designations given by the Arathi:

| Place Name | Notes |
|-----------|-------|
| Velhan's Claim | Arathi personal name + Common possessive |
| The Hungering Pool | Descriptive Common name |
| Bleak Sand | Descriptive Common name |
| The Fangs | Descriptive Common name |
| Sunless Strand | Descriptive Common name |
| The Undersea | Descriptive Common name |
| Morgaen's Tears | Arathi personal name + Common possessive |
| Salien Gutters | Nerubian territory (City of Threads) |

## Cross-Family Phonological Comparison

| Feature | Murloc (Nerglish) | Jinyu | Ankoan | Kobyss |
|---------|-------------------|-------|--------|--------|
| Syllable structure | Syllabic consonants; mrgl- patterns | Full CV/CVC | Full CV/CVC | English compounds only |
| Vowel inventory | Restricted (mostly /ɜ/, /ʌ/) | Full /a e i o u/ | Full /a e i o u/ | N/A (no native names) |
| Consonant inventory | /m g r l b k/ | Full including /ʃ/ | Full (smaller sample) | N/A |
| Name length | 1–4 "beats" (mrgl-glr) | 2–5 syllables | 2–4 syllables | English compounds |
| Apostrophe usage | None | Sele'na (1 attestation) | None | None |
| Attested native words | *gmmmlmrmrgmg*, *Mmmurlok* | *Onoho* ("Deep Shadow") | None | None |
| Proper names | Gurgling compounds | Non-English proper names | Non-English proper names | English descriptors only |

## Etymological Speculation: "Kobyss"

1. **Real-world "abyss"**: *kobyss* ≈ "co-abyss" or "of the abyss" — a name given by the Arathi based on the creatures' emergence from deep waters
2. **Ankoan-language origin**: If kobyss descend from ankoan, *Ko-byss* could be a two-syllable ankoan word, but /ks/ cluster in *-byss* doesn't match typical ankoan phonotactics
3. **Nerglish substrate**: The *-yss* ending could reflect gurgling phonology, but *ko-* is not typical of Nerglish onsets
4. **Independent Arathi designation**: "Kobyss" may simply be an Arathi word, unrelated to any known language family

**Confidence: Very Low** — no native kobyss words attested.

## Cross-Nerubian Contact

The Gutter tribe inhabits the Salien Gutters in the City of Threads (nerubian territory in Azj-Kahet). The nerubian alchemist **Al'kubian** studied this tribe for weeks, noting they grew stronger by consuming Ascended nerubians. This provides the only known direct linguistic contact between kobyss and nerubians.
